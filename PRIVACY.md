...
- **AI reading help (optional):** the "Summary" and "Simplify" helpers in Reader View run only where your browser has on-device AI (Chrome's built-in Summarizer / Rewriter / Prompt API) — nothing leaves your device, and ReadTune adds no network access for this feature. Where a browser doesn't have that yet, the feature just isn't available there; nothing is sent anywhere as a fallback. A rewrite can be wrong and is always shown beside the original.
