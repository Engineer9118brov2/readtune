...
- **The default voice ships inside the extension.** `lib/piper/voices/en_US-ljspeech-medium.onnx`
  ...
  any network fetch. If that bundled asset is unavailable, read-aloud fails
  clearly rather than attempting a remote fallback — so first-use read-aloud
  works offline, with no download and no permission prompt.
...
- **Zip size: ~60 MB** (`readtune-0.9.5.zip`). The bundled voice is
...
should swap espeak-ng for a permissively licensed g2p to remove the GPL review
question. The current package includes the GPL text and a source-offer notice in
`lib/piper/espeak-ng.LICENSE.txt`; obtain legal review before treating that as a
commercial distribution determination.
...
shared/piper/piper-tts-web.js   vendored @mintplex-labs/piper-tts-web glue, patched:
                          ...
                          from chrome.runtime.getURL before any network fetch; no CDN
                          WASM defaults or bundled-model network fallback remain.
...
- `piper_phonemize` bundles **espeak-ng — GPLv3-or-later.** The repository
  includes distribution materials, with a known long-term cleanup:
  - `lib/piper/espeak-ng.LICENSE.txt` ships the full GPLv3 text, a written offer
    ...
    wholesale).
  - espeak-ng is compiled to a standalone wasm module with its own linear
    memory; ReadTune calls it across that boundary (text in, phoneme string out)
    and does not link its internals. That implementation detail is documented
    for a reviewer; it is not a legal conclusion about the extension's overall
    licensing.
  ...
  - **Cleanup:** replace espeak-ng with a permissively licensed
    grapheme-to-phoneme step (small English rules-based g2p, or a tiny learned
    model) before a commercial release where legal review is unavailable.
