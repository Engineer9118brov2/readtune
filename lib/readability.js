...
Readability.prototype = {
  ...
  _fixRelativeUris(articleContent) {
    ...
    this._forEachNode(links, function (link) {
      ...
      if (href) {
        // Remove executable/embedded URL schemes. Reader View runs content in
        // an extension page, so javascript:, vbscript:, and data: links from
        // untrusted article markup must never survive as clickable anchors.
        var unsafeHref = href.trim().toLowerCase();
        if (
          unsafeHref.indexOf("javascript:") === 0 ||
          unsafeHref.indexOf("vbscript:") === 0 ||
          unsafeHref.indexOf("data:") === 0
        ) {
